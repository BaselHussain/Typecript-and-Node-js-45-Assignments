...
var current_users = ["Rameesha", "Ayesha", "Zayyan", "Mubashir", "Huzaifa"];
var current_users_lower = current_users.map(function (curVal) { return curVal.toUpperCase(); });
// • Make another list of five usernames called new_users. Make sure one or two of the new usernames are also in the current_users list.
var new_users = ["MUBASHIR", "Huzaifa", "Shaheer", "Zia", "Riaz"];
// • Loop through the new_users list to see if each new username has already been used. If it has, print a message that the person will need to enter a new username. If a username has not been used, print a message saying that the username is available.
for (var i = 0; i < new_users.length; i++) {
    if (current_users_lower.includes(new_users[i].toUpperCase())) {
        console.log("Sorry ".concat(new_users[i], " You will need to enter a new username"));
    }
    else {
        console.log("".concat(new_users[i], ", this username is available"));
    }
}
...
function make_shirt(size, message) {
    ...
    console.log("The size of the shirt is ".concat(size, " and the message printed on it will be \"").concat(message, "\""));
}
make_shirt();
make_shirt("medium");
